...
[Rotation("Kirbo's VPR", CombatType.PvE, GameVersion = "7.05")]
[SourceCode(Path = "main/DefaultRotations/Melee/VPR_Default.cs")]
[Api(4)]
public sealed class VPR_Kirbo : ViperRotation
{
    ...
    protected override bool EmergencyAbility(IAction nextGCD, out IAction? act)
    {
        ...
        if (SerpentCombo == 0 && BurstMed && SerpentsIrePvE.EnoughLevel && SerpentsIrePvE.Cooldown.ElapsedAfter(115)
    ...
    {
        if (SerpentCombo == 0 && SecondWindPvE.CanUse(out act)) return true;
        if (SerpentCombo == 0 && BloodbathPvE.CanUse(out act)) return true;
    ...
    {
        if (SerpentCombo == 0 && FeintPvE.CanUse(out act)) return true;
    ...
    {
        if (SerpentCombo == 0 && ArmsLengthPvE.CanUse(out act)) return true;
    ...
    {
        if (SerpentCombo == 0 && LegSweepPvE.CanUse(out act)) return true;
    ...
    protected override bool GeneralGCD(out IAction? act)
    {
        ...
        if ((MaxRattling == RattlingCoilStacks || RattlingCoilStacks >= MaxUncoiledStacksUser) && !Player.HasStatus(true, StatusID.ReadyToReawaken) && SerpentCombo == 0)
        {
            if (UncoiledFuryPvE.CanUse(out act, usedUp: true)) return true;
        }

        if (BurstUncoiledFury && Player.HasStatus(true, StatusID.Medicated) && !Player.HasStatus(true, StatusID.ReadyToReawaken) && SerpentCombo == 0)
        {
            if (UncoiledFuryPvE.CanUse(out act, usedUp: true)) return true;
        }

        //Uncoiled fury use
        if (SerpentsIrePvE.Cooldown.JustUsedAfter(30) && !Player.HasStatus(true, StatusID.ReadyToReawaken) && SerpentCombo == 0)
        {
            if (UncoiledFuryPvE.CanUse(out act, usedUp: true)) return true;
        }
        ...
        //Ranged
        if ((UFGhosting || (!UFGhosting && SerpentCombo == 0)) && UncoiledFuryPvE.CanUse(out act, usedUp: true)) return true;
        if ((UFGhosting || (!UFGhosting && SerpentCombo == 0)) && WrithingSnapPvE.CanUse(out act)) return true;
