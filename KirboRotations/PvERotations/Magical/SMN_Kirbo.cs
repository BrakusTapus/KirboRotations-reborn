...
public sealed class SMN_Kirbo : SummonerRotation
{
    ...
    [RotationConfig(CombatType.PvE, Name = "Hold Searing Light when in ultimates")]
    public bool Hold0GCD { get; set; } = false;
    ...
    [RotationConfig(CombatType.PvE, Name = "Use Swiftcast on Ifrit")]
    public bool AddSwiftcastOnIfrit { get; set; } = false;
    ...
    protected override bool AttackAbility(IAction nextGCD, out IAction? act)
    {
        ...
        act = null;
        if (Hold0GCD && HostileTarget?.CurrentHp <= 200000)
        {
            return false;
        }
    ...
    protected override bool EmergencyAbility(IAction nextGCD, out IAction? act)
    {
        ...
        if (!AddSwiftcastOnGaruda && AddSwiftcastOnIfrit && nextGCD == RubyRitePvE && Player.Level > 86 && !InBahamut && !InPhoenix && !InSolarBahamut)
        {
        ...
        if (!AddSwiftcastOnIfrit && AddSwiftcastOnGaruda && nextGCD == SlipstreamPvE && Player.Level > 86 && !InBahamut && !InPhoenix && !InSolarBahamut)
        {
            if (SwiftcastPvE.CanUse(out act)) return true;
        }
    ...
    protected override bool GeneralGCD(out IAction? act)
    {
        ...
        if (AddSwiftcastOnIfrit && !AddSwiftcastOnGaruda && RubyRitePvE.CanUse(out act, skipAoeCheck: true, skipCastingCheck: !SwiftcastPvE.Cooldown.IsCoolingDown || HasSwift)) return true;
        if (!AddSwiftcastOnIfrit && AddSwiftcastOnGaruda && SlipstreamPvE.CanUse(out act, skipAoeCheck: true, skipCastingCheck: !SwiftcastPvE.Cooldown.IsCoolingDown || HasSwift)) return true;
